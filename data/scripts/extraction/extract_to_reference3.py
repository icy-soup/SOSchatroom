...
def extract_dialogues(data, filename):
    ...
    for line in lines:
        for code, name in CHAR_MAP.items():
            # Use search not match - the line may have binary prefix
            m = re.search(re.escape(code) + r'\d{6,}.[^\x00-\x1f]*「([^」]*)」', line)
            if m:
                q = m.group(1).strip()
                if len(q) >= 2:
...
for i in range(len(all_data) - 1):
    curr = all_data[i]['speaker']
    nxt = all_data[i + 1]['speaker']
    if curr != nxt:
        key = f"{curr}→{nxt}"
        transitions[key] = transitions.get(key, 0) + 1

print("\n=== Top transitions ===")
for key, n in sorted(transitions.items(), key=lambda x: -x[1])[:30]:
    print(f"  {key}: {n}")
